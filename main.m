...
%find max real part of well-resolved modes
[ReSmax,kReSmax] = max(real(sm(Resolved)));
...
%max(real(sv))
hold on
plot(real(sm),imag(sm),'o')% col')
plot(real(sm(Resolved)),imag(sm(Resolved)),'x')
xlabel('real(s)')
ylabel('imag(s)')
